...
/**
	...
	It does so by counting each in. New lines will reset the column
...
public:
	...
	void countCharacter(char in) {
		/**
			We need to count UTF-8 monospace in length (the amount of code points). This is the perceived.
			...
			many of the following ins we have to discard because they're part of the same in.

			We will assume that valid UTF-8 is put into this function. Note that some in sequences make
			the string look smaller than it actually is: "٩(͡๏̯͡๏)۶   ٩(-̮̮̃•̃)". does this, for example.
			We do not need to get that involved into exact in counting. What we do here count the
		...
		*/
		...
		if (static_cast<unsigned char>(in) & 0b10000000)
		{
			if (static_cast<unsigned char>(in) & 0b01000000)
			{
				if (static_cast<unsigned char>(in) & 0b00100000)
				{
					if (static_cast<unsigned char>(in) & 0b00010000)
					{
						if (static_cast<unsigned char>(in) & 0b00001000)
						{
							if (static_cast<unsigned char>(in) & 0b00000100)
							{
								utf_bytes_left = 5;
							}
							else
