...
for the non-c++ crowd). Sometimes though, C++ compilers perform a long time processing templates during compile-time.
...
(from here on referred to as module) modules do not have any other data that might make them unequal. For example, if an object read from an implicit variable, or from a socket. We can not guarantee modular purity in those cases.
...
*Result*: Function signatures are to be written in the following form.
